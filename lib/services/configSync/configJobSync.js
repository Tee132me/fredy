...
import { config, refreshConfig } from '../../utils.js';
 ...
 */
function syncJobToDatabase(jobConfig) {
  try {
    if (!jobConfig.id) {
    ...
    // If no notification adapters, use the global adapters from config
    if (notificationAdapters.length === 0 && config.adapters) {
...
async function syncConfigJobsToDatabase() {
  ...
  try {
    ...
    replaceEnvPlaceholdersSimple(config);
    ...
    for (const jobConfig of configJobs) {
      if (syncJobToDatabase(jobConfig)) {
        syncedCount++;
      } else {
        skippedCount++;
      }
