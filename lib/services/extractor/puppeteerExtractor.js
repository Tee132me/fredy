...
export default async function execute(url, waitForSelector, options) {
  ...
  try {
    ...
    browser = await puppeteer.launch({
      ...
      protocolTimeout: 180000, // 3 mins protocol timeout (Railway-optimized)
      userDataDir,
    ...
    // Navigation - optimized for Railway environment
    const response = await page.goto(url, {
      waitUntil: options?.waitUntil || 'domcontentloaded',
      timeout: 90000 // 90 seconds (balanced for reliability)
    });
    ...
    if (waitForSelector != null) {
      // Reduced selector timeout for faster failures (Railway-optimized)
      const selectorTimeout = options?.puppeteerSelectorTimeout ?? options?.puppeteerTimeout ?? 45_000;
