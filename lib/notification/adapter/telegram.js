 ...
 * @param {string} [o.link]
 * @param {Array<number>} [userIds] - Optional user IDs to embed as hashtags
 * @returns {string}
 */
function buildCaption(jobName, serviceName, o, userIds = []) {
  const title = shorten((o.title || '').replace(/\*/g, ''), 90);
  const meta = [o.address, o.price, o.size].filter(Boolean).join(' | ');
  let caption = `<i>${escapeHtml(jobName)}</i> (${escapeHtml(serviceName)})\n<a href='${escapeHtml(
    o.link || '',
  )}'><b>${escapeHtml(title)}</b></a>\n${escapeHtml(meta)}`;
  
  // Append user ID hashtags if present (for MyImmoBuddy bot routing)
  if (Array.isArray(userIds) && userIds.length > 0) {
    const userTags = userIds.map(id => `#U${id}`).join(' ');
    caption = `${caption}\n\n${userTags}`;
  }
 ...
 * @returns {string}
 */
function buildText(jobName, serviceName, o, userIds = []) {
  const title = shorten((o.title || '').replace(/\*/g, ''), 90);
  const meta = [o.address, o.price, o.size].filter(Boolean).join(' | ');
  let text = (
 ...
 * - Embeds user IDs as hashtags for bot routing (MyImmoBuddy integration)
 *
 * @param {Object} params
...
export const send = ({ serviceName, newListings = [], notificationConfig, jobKey }) => {
  ...
  // Extract user IDs from job metadata (for MyImmoBuddy bot routing)
  const userIds = job?.user_ids || [];
  if (userIds.length > 0) {
    logger.debug(`Job '${jobName}' has ${userIds.length} user ID(s) for routing: ${userIds.join(', ')}`);
  }
  ...
  const promises = newListings.map(async (o) => {
    ...
    const textPayload = {
      chat_id: chatId,
      text: buildText(jobName, serviceName, o, userIds),
      parse_mode: 'HTML',
    ...
    return await throttledCall('sendPhoto', {
      chat_id: chatId,
      photo: img,
      caption: buildCaption(jobName, serviceName, o, userIds),
      parse_mode: 'HTML',
      ...(message_thread_id ? { message_thread_id } : {}),
    }).catch(async (e) => {
