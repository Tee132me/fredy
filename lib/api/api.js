...
import { fredyJobRouter } from './routes/fredyJobRouter.js';
const service = restana();
const staticService = files(path.join(getDirName(), '../ui/public'));
// Railway/production environments use PORT env var, fallback to config.port or 9998
const PORT = process.env.PORT || config.port || 9998;

service.use(bodyParser.json());
service.use(cookieSession());
service.use(staticService);

// Health check endpoint (no auth required) - for Railway monitoring
...
// External API for MyImmoBuddy integration (no auth interceptor - uses API key)
service.use('/api/external', fredyJobRouter);
